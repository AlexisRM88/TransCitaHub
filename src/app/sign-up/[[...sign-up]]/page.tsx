...
import { ShieldCheck, Mail, Lock, User as UserIcon, AlertCircle, ArrowRight, Car, MousePointer2 } from "lucide-react";
...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Flow signUp requires name, email and password
      await signIn("password", { name, email, password, flow: "signUp" });
      router.push("/");
    } catch (err: any) {
      console.error(err);
      setError("No se pudo crear la cuenta. Verifica tus credenciales.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-gray-950 via-gray-900 to-green-950 text-white font-sans overflow-hidden relative">
      ...
      <div className="absolute top-[-10%] right-[-10%] w-[40%] h-[40%] bg-green-500/10 blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="max-w-md w-full relative z-10">
        <div className="bg-gray-900/80 backdrop-blur-2xl border border-gray-800 rounded-[2.5rem] shadow-2xl p-8 sm:p-10">
          <div className="flex flex-col items-center mb-8 text-center">
            ...
            <h1 className="text-3xl font-extrabold mb-2 bg-gradient-to-r from-green-400 to-emerald-300 bg-clip-text text-transparent">Nuevo Acceso</h1>
            <p className="text-sm text-gray-400 font-medium">Crea tu cuenta de colaborador.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-xs font-black uppercase tracking-widest text-gray-400 px-1">Nombre Completo</label>
            ...
            <div className="space-y-1">
              <label className="text-xs font-black uppercase tracking-widest text-gray-400 px-1">Correo Escolar/Trabajo</label>
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-500 group-focus-within:text-green-500 transition-colors">
                  <Mail size={18} />
                </div>
            ...
            <div className="space-y-1">
              <label className="text-xs font-black uppercase tracking-widest text-gray-400 px-1">Crear Contraseña</label>
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-500 group-focus-within:text-green-500 transition-colors">
                  <Lock size={18} />
                </div>
          ...
          <div className="mt-8 text-center text-sm font-medium text-gray-500">
            ¿Ya tienes cuenta?{" "}
            <Link href="/sign-in" className="text-green-400 hover:text-green-300 font-bold transition-colors">
              Inicia Sesión
            </Link>
          </div>
        </div>
